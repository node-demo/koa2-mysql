$(function () {
  ...
  oShadow.find('.emoji li').on('click', function () {
    ...
    var res = oShadow.find('textarea[name=text]').val() + emoji;
    oShadow.find('textarea[name=text]').val(res);
    console.log(i);
  });

  var aIcon = ['大哭', '害羞', '憨笑', '奸笑', '可爱', '玫瑰', '难过', '太阳', '调皮', '偷笑', '吻', '握手', '疑问', '拥抱', '再见', '真棒'];
  ...
  // 发布话题字数
  oShadow.find('textarea[name=text]').on('keyup', function () {
    show_key($(this));
  });
  oShadow.find('textarea[name=text]').on('blur', function () {
    show_key($(this));
  });
